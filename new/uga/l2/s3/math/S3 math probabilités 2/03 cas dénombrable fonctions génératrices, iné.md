...
Maintenant dans le cas générale, la définition d’une *fonction génératrice de probabilités* $f$ d’une V.A $X \in\N$ est la suivante. Notons qu’on peut le faire même avec une infinité de valeurs possibles pour la V.A. $X$ !

$$
f(z)=\mathbb{E}[z^X]=\sum_{k=0}^\infin (z^k \cdot 
\mathbb P(X=k)) = 
\newline
\text{}
\newline
...
Notons que $f(1) = \sum_{k=0}^\infin \mathbb P(X=k) = 1$. C’est simplement la somme des probabilités.

Il y a deux propriétés importantes si on vérifie que $|z| ≤ 1$ :

- $f(z)$ est bien définie, càd. $f(z)\in\R$, car elle serait inférieur ou égale à la série $\sum(\mathbb P(X=k))$ qui on sait que converge à $1$. Ceci découle des propriétés des séries géométriques (on pourrait voir quelques polynômes comme des suites géométriques).
...
$$
\mathbb{E}[z^{\sum_{i=1}^n X_i}] = (\mathbb{E}[z^{X_k}])^n \text{, où } k \in \N \text{ et } 1 \le k \le n
$$
...
$$
f'(1)=\sum_{k=0}^\infin (k\cdot \mathbb P(X=k))=\mathbb{E}[X] \text{, et}

\newline
\text{}
\newline

f''(1)= \sum_{k=0}^\infin((k)(k-1)\cdot \mathbb P(X=k)) \implies f'(1) + f''(1) = \mathbb{E}[X^2]
$$
...
Soit $(X_n)$ une suite des VA entières non-négatives représentés chacune par la fonction génératrice $f_n$, et $X$ une VA entière non-négative de fonction génératrice $f$. Les deux propositions sont équivalentes, avec $k\in\N$ et $0 \le z \le 1$ :

$$
\lim_{n \rightarrow \infin} \mathbb{P}(X_n=k) = \hspace{-1pt}\overbrace{\mathbb{P}(X=k)}^\text{si l'une existe...} \iff \lim_{n \rightarrow \infin} f_n(z) = \hspace{-8pt}\overbrace{f(z)}^\text{l'autre aussi}
$$
...
Ici, on a démontré qu’il existe effective la limite quand $n \rightarrow \infin$ de $f_n(z)$, laquelle on va appeler tout simplement $f(z)$. Donc, par l’équivalence de propositions précédente, on a démontré aussi qu’il existe aussi la limite de la loi de probabilités $\mathbb{P}(X_n=k)$ quand $n \rightarrow \infin$, sans besoin d’expliciter son expression mathématique.
...
$$
0 \le \lim_{n \rightarrow \infin} \mathbb{P} \left(\bar{X}_n -\mathbb{E}[X_i]\ge \epsilon\right), \text{ par définition des probabilités.}
