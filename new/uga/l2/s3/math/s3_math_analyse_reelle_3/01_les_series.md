...
- La ***limite*** de la série est $\Sigma_{k=0}^{\infin} u_n  := \lim_{n \rightarrow \infin} \Sigma_{k=0}^n u_n$. C’est un nombre réel.

# Suites, fonctions et quelques limites

## Dériver une suite à partir d’une fonction $f$ sur $\R$
...
## La convergence comme le terme limite de la suite : $“u_\infin”$
...
- Corollaire : si $\lim_{n→\infin} \frac{u_n}{v_n}$ = $\ell,\text{ } \ell \in \R$, donc $\Sigma u_n$ et $\Sigma v_n$ sont de même nature (conv./div.).
- Corollaire : si on a deux séries $(u_n)$ et $(v_n)$, si le quotient $\lim_{n \rightarrow \infin} \frac{u_n}{v_n} = 1$, on dit que le séries sont *équivalentes.*
...
Les séries de Riemann sont les séries de la forme $\Sigma \frac{1}{n^p}$, avec $p \in \R$.
