...
On dit qu'une suite $(u_n)$ est ***convergente*** si elle admet une limite $\ell ∈ \mathbb{R}$. Dans le cas contraire, on dit que $(u_n)$ est ***divergente***. Quand la limite est "l'infini", on utilise la définition de inexistence d'une borne (supérieur ou inférieur).

- Pour $\infty$ : $∀M ∈ \mathbb{R}, ∃N ∈ \mathbb{N}, ∀n ≥ N,  u_n ≥ M$
- Pour $-\infty$ : $∀M ∈ \mathbb{R}, ∃N ∈ \mathbb{N}, ∀n ≥ N, u_n ≤ M$

## Propriétés

- Si $(u_n)$ admet une limite $\ell ∈ \mathbb{R}$, alors cette limite est unique.
- Soit $(u_n)$ une suite. Alors $(u_n)$ converge vers $\ell ∈ \mathbb{R}$ si et seulement si la suite $(u_n − \ell)$ converge vers $0$.
- Soit $(u_n)$ une suite et soit $\ell ∈ \mathbb{R}$...
    - S'il existe une suite $(v_n)$ convergeant vers $0$ et si $∀n ∈ \mathbb{R}, |u_n − \ell| ≤ v_n$, alors $(u_n)$ converge vers $\ell$.
...
Soient $(u_n)$ et $(w_n)$ des suites convergeant vers la même limite $\ell ∈ \mathbb{R}$, Si $(v_n)$ est une suite vérifiant $u_n ≤ v_n ≤ w_n$ à partir d'un certain rang, alors $(v_n)$ est convergente et $\lim v_n = \ell$.
...
Sous toutes ces conditions, elles convergent vers la même limite $\ell ∈ \mathbb{R}$, qui, de plus, vérifie ce qui suit : $∀n ∈ \mathbb{N}, u_n ≤ \ell ≤ v_n$.

### Suite extraite

Prenons comme base une suite de termes de la suite $(u_n)$. Formellement, c'est la donnée d'une suite $(u_{k_n}), n∈\mathbb{N}$ pour des indices ${k_n} ∈ \mathbb{N}$ vérifiant $k_0 < k_1 < . . .$
...
Soit $I$ un intervalle fermé et borné de $\mathbb{R}$ et soit $f : I → I$. On dit que $f$ est *contractante* sur $I$ si :

$$
∃k ∈ [0, 1[, ∀x, y ∈ I, |f(x) − f(y)| < k|x − y|
$$

En plus, si $f$ est contractante, alors $f$ est continue (admis) et possède un unique point fixe $\ell ∈ \mathbb{R}$ (ceci découle du théorème du point fixe de Banach). De plus, pour tout $u_0 ∈ I$, la suite définie par $u_0$ et $u_{n+1} = f(u_n)$ converge vers $\ell$.
