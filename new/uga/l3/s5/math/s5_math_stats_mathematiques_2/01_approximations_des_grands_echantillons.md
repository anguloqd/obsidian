...
C’est le cas si la suite des répartitions de $X$, $\big(F_{X_n}\big)$, converge à une fonction existante, appelée $F_{X_\infin}$ ou tout simplement $F_X$. Notons que les propositions avec les fonctions génératrice $\mathcal{G}_{X_n}$ et $\mathbb E[f(X_n)]$ sont analogiques.
...
- **Théorème de Slutsky**.
Si $X_n → X$ presque sûrement et $g$ est une fonction continue sur $\R$, alors $g(X_n) → g(X)$ presque sûrement.
- **Théorème de Slutsky “en deux dimensions/variables”**.
Si $X_n → X$ presque sûrement et $Y_n → Y$ presque sûrement et $g$ est une fonction continue sur $\R^2$ alors $g(X_n, Y_n) → g(X, Y)$ presque sûrement.
