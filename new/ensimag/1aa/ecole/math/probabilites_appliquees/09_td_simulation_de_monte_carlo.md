...
TCL : la variable z_n = \frac{\bar X_n - \mu}{\sigma / \sqrt{n}} a pour limite en loi lim_{n\to\infty} P(Z_n \le x) = \int_{-\infty}^{\infty} \frac{1}{\sqrt{2\pi}} \exp(-\frac{t^2}{2})) dt, qui est \mathcal N(0,1).

Vitesse de convergence : \bar{X}_n \to \mu quand n \to \infty à la même vitesse que \frac{\sigma}{\sqrt n} \to 0 quand n \to +\infty. L’ordre de convergence de \bar{X}_n \to \mu c’est de o(\frac{1}{\sqrt n}).
