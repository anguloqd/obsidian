...
- Cette élaboration est effectuée à partir de deux sources. D'une part, les connaissances préalables dont vous disposez déjà en mémoire ; d'autre part, les informations provenant de la situation.
...
- La prise d’information est par conséquent ici conçue comme un ***processus actif et anticipatif***, et pas simplement comme une réaction passive d’un système cognitif soumis aux stimulations de l’environnement extérieur.
...
- **Modèle TOTE** : test operate, test exit.
    ...
    - Si le test est négative, alors le système modifie soit le stimulus ou soit le même système pour aboutir au positif.
    - Nature récursive jusqu'à la réponse positive ou il n'y a plus de modifications à faire, dans ce cas on reçoit une réponse différente à l'attendu.
    - C'est finalement un modèle qui explique l’action d’un système qui est déclenchée par le fait d’une différence (une incongruité) entre l’état du système et l’état attendu (l’objectif) de ce même système.
- Par ce processus cognitif, la personne est tout d’abord amenée à comparer l’état présent de sa situation par rapport à la situation idéale dans laquelle elle désire se trouver (test).
- La dissonance identifiée entre les deux va dès lors engendrer la mise au point d’un plan selon lequel elle va agir (operate).
- A la suite de cette action elle va de nouveau tester la situation dans laquelle l’a conduite son action (test).
- Si l’information recueillie lui permet de conclure qu’elle a atteint la situation idéale, l’action cesse (exit), sinon la personne va poursuivre son action (operate), jusqu’à l’obtention des conditions de fonctionnement désirées.
- Penser, c’est raisonner. Et raisonner, c’est appliquer les lois rigoureuses et universelles de la logique.
- La forme canonique de cette logique classique est le syllogisme, raisonnement déductif qui permet de parvenir à une conclusion assurée à partir de prémisses :
    - « Socrate est un homme ; tous les hommes sont mortels ; donc Socrate est mortel ».
- La cognition affecte la conduite, si on comprend la cognition comme le stockage et traitement d'info.
- Tous les autres facteurs qui affectent la conduite et ne font pas partie de la cognition sont appelés " **conation** ".  Par exemple, les émotions et la motivation.
...
- Chaque être humain est **le produit de deux histoires** s'inscrivant dans des gammes de temps très différentes :
    ...
    - **Phylogenèse** : dont l'unité de temps est de l'ordre du million d'années.
- **Ontogenèse** :
    - Les gènes constituent en fait une recette pour le développement.
    - Dans la recette d'un gâteau, les ingrédients sont spécifiés dans les instructions pour faire le gâteau. La qualité et la fraîcheur des ingrédients, la température du four, vont cependant avoir un effet sur le résultat final.
    - La recette, c'est le code génétique héréditaire, et les autres variables sont l'équivalent des conditions environnementales dans lesquelles se déroule le développement.
    - Comme le remarque fort justement Dawkins, une fois sorti du four, il n'existe « aucun procédé qui permette de corréler point par point les mots de la recette aux miettes de gâteau et réciproquement »
- **Phylogenèse** :
    - Dans la longue phase préhistorique, un grand nombre de schémas comportementaux innés auraient été façonnés par la sélection naturelle.
    - Cet environnement était principalement déterminé par le mode de vie et la démographie des populations humaines du Pléistocène, c'est-à-dire à des groupes de quelques dizaines à 300 chasseurs-cueilleurs.
    - Dans un tel environnement que se seraient construits, des schémas comportementaux répondant à des problèmes adaptatifs.
