...
$$\underbrace{P(t\in\mathcal C|H_1)}_{1-\beta}\ge \underbrace{P(t\in\mathcal C|H_0)}_\alpha$$
...
$$\phi(\theta)=P(t\in\mathcal C|\theta)$$

Un test est dit *sans biais* si sa puissance est supérieure ou égale à son risque $\alpha$, donc

$$P(t\in\mathcal C|H_1)\geP(t\in\mathcal C|H_0) \iff 1-\beta\ge\alpha$$
...
I(\theta|\mathbf{x})=\mathbb E\left[\left( \frac{\partial \ln L}{\partial\theta}(\theta)-\cancel{E(s(\theta)]}^{\space0}\right)^2\right]=\int_\Omega \left( \frac{\partial \ln L}{\partial\theta}(\theta)\right)^2L(\theta|\mathbf{x})d\theta$$
...
$$\sup_{\theta\in\Theta_0} P(\lambda(\mathbf{x})<k)=\alpha$$
...
### Supposition de même variance, inconnue $(
$$
t$-test de variance groupée$)$
...
### Supposition de variances différentes, inconnues $(
$$
z$-test asymptotique$)$
...
#### Supposition de variances différentes, connues $(
$$
z$-test proprement$)$
...
### Comparaison de $\sigma^2_1$ et $\sigma^2_2$ $(
$$
F$-test ou test d’ANOVA$)$
...
\right]
$$
### Comparaison de $p_1$ et $p_2$, asymptotique $(
$$
z$-test$)$
