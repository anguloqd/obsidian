...
$$E(\bar X) = \mu\hspace{8pt}\text{ et\hspace{8pt} Var}(\bar X)=\frac{\sigma^2}{n}$$
...
$$E(S^{2^\prime})=\sigma^2 \hspace{8pt}\text{et}\hspace{8pt}V(S^{2^\prime})=\frac{1}{n}\left(\mu^\prime_4-\frac{n-3}{n-1}\sigma^4\right), \text{où }\mu^\prime_4=E((X-\bar X)^4]$$
...
$$\lim_{n\rightarrow\infty} nV(\hat\kappa_{\lfloor np\rfloor +1})=\frac{p(1-p)}{f(\kappa_p)^2}
...
$$
P(\ell_1\le n_\text{inf} \le \ell_2) = \sum_{k=\ell_1}^{\ell_2} \binom{n}{k}p^k(1-p)^{n-k}
$$
Les $\ell_1$ et $\ell_2$ sont des limites pour encadrer. Ici, on veut se fixer deux tâches :

- On veut choisir leurs valeurs tel que $P(\ell_1\le n_\text{inf} \le \ell_2) \ge 0.95$ et quand même bien proche aussi de $0.95$.
...
$$
P(\ell_1\le n_\text{inf} \le \ell_2) = P(X_{(\ell_1)}\le\kappa_p\le X_{(\ell_2+1)}),
$$
et donc
$$
P(X_{(\ell_1)}\le\kappa_p\le X_{(\ell_2+1)})=\sum_{k=\ell_1}^{\ell_2} \binom{n}{k}p^k(1-p)^{n-k}
$$
...
Juste avant de commencer, on va faire une modification importante à l’espérance $E(X)$ ou plutôt la moyenne $\mu$ dans ce contexte :
$$
...
$$
On montre que, lorsque $M$ tend vers l’infini, l’estimateur issu de cette procédure tend presque sûrement vers l’estimateur du maximum de vraisemblance du paramètre $V(\hat\theta)$. **Oui, il faut rappeler que $V(\hat\theta)$ est un paramètre**.
...
&\spaceE(\hat f_n(x)]=\frac{1}{nh}E(n_i)=\frac{np_i}{nh}=\frac{p_i}{h}
...
$$
E(\hat f(x)]=f(x), \text{ et voyons que } E(\hat f_n(x)]=\frac {p_i} h
$$
...
E(\hat f(x)]-f(x)=\frac{h^2}{2}f^{\prime\prime}(x)\int_\mathbb{R} u^2K(u)du+o(h^2), \text{ où }u=\frac{x-t}{h}
...
$$
V(\hat f(x))=\frac{1}{nh}f(x)\int_\mathbb{R} K(u)^2du+O\left(\frac 1 n\right)
$$
...
\lim_{n\rightarrow\infty}P(\sqrt{n}D_n<x)=
...
$$
P(\sqrt{n}\sup_{x\in\mathbb{R}}|\hat F_n-F(x)|< g_{95\%})\simeq95\%
$$
Mais, voyons que
$$
...
$$
E(\hat F_n(x)-F(x)]=\frac {h^2}2 f\prime(x)\int_\mathbb{R} u^2K(u)du+o(h^2)
$$
