...
$$E(\hat{\beta}_0)=\beta_0 \text{\hspace{8pt}et\hspace{8pt}}E(\hat{\beta}_1)=\beta_1$$

    
- Les variances sont les suivantes, mais il nous manque un paramètre $\sigma^2_u$.
    

$$V(\hat{\beta}_1)=\frac{\sigma^2_u}{\sum_{i=1}^N(x_i-\bar{x})^2}

\text{\hspace{8pt}et\hspace{8pt}}

V(\hat{\beta}_0)=\frac{\sigma^2_u}{n}\frac{\sum_{i=1}^Nx_i^2}{\sum_{i=1}^N(x_i-\bar{x})^2}$$
...
$$\hat{\sigma}^2_u=\frac{\sum_{i=1}^n\hat{u}_i^2}{N-(k+1)} \implies E(\hat{\sigma}^2_u)=\sigma^2_u$$
...
$$V(\hat{\boldsymbol{\beta}})=\hat{\sigma^2_u}(\mathbf{X}^T\mathbf{X})^{-1}=

\begin{bmatrix}

V(\hat{\beta}_0) & \text{Cov}(\hat{\beta}_0, \hat{\beta}_1) & \cdots & \text{Cov}(\hat{\beta}_0, \hat{\beta}_k)

\\

\text{Cov}(\hat{\beta}_1, \hat{\beta}_0) & V(\hat{\beta}_1) & \cdots & \text{Cov}(\hat{\beta}_1, \hat{\beta}_k)
...
\text{Cov}(\hat{\beta}_k, \hat{\beta}_1) &\text{Cov}(\hat{\beta}_k, \hat{\beta}_2) &\cdots &V(\hat{\beta}_k)
