...
$$V(W) = \text{Var} \left( \sum_{i=1}^{n_X} R_i \right) = \sum_{i=1}^{n_X} V(R_i) + \sum_{\substack{i,j=1 \\ i \ne j}}^{n_X} \text{Cov}(R_i, R_j)
\\
...
$$V(W) = n_X \cdot \frac{n^2 - 1}{12} + n_X(n_X - 1) \cdot \left( -\frac{n+1}{12} \right)$$
...
$$W = \sum_{i=1}^{n_X} R_i, \quad E(W) = n_X \cdot \frac{n+1}{2}, \quad V(W) = \frac{n_X n_Y (n+1)}{12}$$
