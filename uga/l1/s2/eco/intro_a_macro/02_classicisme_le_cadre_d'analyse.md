...
- L'économie est considérée fermée
...
- La fonction qui décrit la production d'une certaine firme est $Y_i = f(K_i, L_i)$
  - $K$ : Capital
  - $L$ : Travail
- On suppose que le capital est fixe à court terme. Donc : $Y_i = F_i(L_i) \equiv f_i(K^*_i, L_i)$. $K^*$ est une constante
...
La condition de premier ordre donne : $\frac{du/dH}{du/dC} = \frac{w}{p}$
...
>[!note]
>Le travail est un bien inférieur par rapport aux heures. Si le bien était un bien normal, les deux effets bougent dans le même sens.
...
La fonction de profit est $\pi_i(L_i) = pF_i(L_i) - wL_i$

Sa dérivée est donc $p \times \frac{df_i}{dL_i} - w$, laquelle on va poser égale à 0.

On finit donc avec l'égalité $\frac{df_i}{dL_i} = \frac{w}{p}$

Si le salaire augmente, pour maintenir cette dernière égalité, on devra faire augmenter la production marginale par rapport au travail ($\frac{df}{dL}$). Mais, par les suppositions initiales, $\frac{df}{dL}$ est décroissante par rapport à $L$. Donc, on devra diminuer $L$, le travail.
...
- $p$ est le "niveau de prix" (montant de transactions / quantité des transactions, ou aussi le déflateur du PIB tout seul)

On peut réécrire l'équation comme $p = \frac{V}{T} \times M$.
...
- En raisonnant à l'agrégé, la demande globale de monnaie, $M^D$, est proportionnelle au revenu global, $pY$, où $p$ est le niveau de prix et $Y$ est le volume de production
...
- On a donc $Y = \frac{C}{p}$. Cette fonction a une forme d'hyperbole
- $Y$ est décroissante et convexe par rapport à $p$
...
Côté les firmes, elles ont besoin de financement pour réaliser ses investissements.

La condition d'investissement est que la valeur présente des recettes soit supérieure à l'investissement initial.
