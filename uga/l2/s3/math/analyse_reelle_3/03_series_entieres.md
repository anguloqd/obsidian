...
+\infty, \text{ si } \ell=0 \\
0, \text{ si }  \ell\longrightarrow+\infty
\end{cases}$$
...
$$\begin{align*}
S : \: &]-R,R[ \: \mapsto \: \mathbb{R} \\
&x \: \mapsto \: \sum_{n=0}^{+\infty} u_nx^n
\end{align*}$$
...
Cette fonction est dite “développable en $0$” s’il existe une série $\sum u_nx^n$ de rayon de convergence $R \ge r$ tel que $f(x) = \sum_{n=0}^{+\infty}u_nx^n$, pour tout $x \in \space ]-r,r[$.
...
Donc, soit $\alpha \in \mathbb{R} \setminus \mathbb{Z}$ et $|x| < 1$, alors :
