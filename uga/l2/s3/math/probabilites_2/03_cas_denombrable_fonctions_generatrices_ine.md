...
$$f(z)=E(z^X)=\sum_{k=0}^1 (z^k \cdot P(X=k)) = 0.9+0.1z$$
...
$$f(z)=E(z^X)=\sum_{k=0}^\infty (z^k \cdot
P(X=k)) =
\newline
\text{}
\newline
...
$$E(z^{\sum_{i=1}^n X_i}) = \prod_{i=1}^n E(z^{X_i})$$
...
$$E(z^{\sum_{i=1}^n X_i}) = (E(z^{X_k}))^n \text{, où } k \in \mathbb{N} \text{ et } 1 \le k \le n$$
...
$$f'(1)=\sum_{k=0}^\infty (k\cdot P(X=k))=E(X) \text{, et}

\newline
\text{}
\newline

f''(1)= \sum_{k=0}^\infty((k)(k-1)\cdot P(X=k)) \implies f'(1) + f''(1) = E(X^2)$$
...
$$E(X) = f'(1) \hspace{8pt}\text{ et\hspace{8pt} Var}(X) = E(X^2) - E(X)^2 = [f'(1)+f''(1)]-[f'(1)]^2$$
...
$$\lim_{n \rightarrow \infty} P(X_n=k) = \hspace{-1pt}\overbrace{P(X=k)}^\text{si l'une existe...} \iff \lim_{n \rightarrow \infty} f_n(z) = \hspace{-8pt}\overbrace{f(z)}^\text{l'autre aussi}$$
